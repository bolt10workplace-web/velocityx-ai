...
const { sendCopyTradeEmail, sendInvestmentEmail, sendTradeEmail, sendDepositEmail, sendWithdrawalEmail } = require('../mailer');
...
const profileUpload = multer({
  ...
  fileFilter: (req, file, callback) => {
    if (!file.mimetype || !file.mimetype.startsWith('image/')) {
      return callback(new Error('Profile photo must be an image file'));
    }
    callback(null, true);
  }
});
...
const formatStatementDate = (value) => new Date(value || Date.now()).toISOString().slice(0, 10);
const getAccountStatementData = (userId) => {
  const deposits = getDeposits()
    .filter(item => item.userId === userId)
    .map(item => ({ date: formatStatementDate(item.createdAt), type: 'Deposit', icon: 'D', iconClass: 'vx_deposit', amount: Number(item.amount) || 0, status: item.status || 'Pending' }));
  const withdrawals = getWithdrawals()
    .filter(item => item.userId === userId)
    .map(item => ({ date: formatStatementDate(item.createdAt), type: 'Withdrawal', icon: 'W', iconClass: 'vx_withdrawal', amount: -(Number(item.amount) || 0), status: item.status || 'Pending' }));
  const trades = getTrades()
    .filter(item => item.userId === userId)
    .map(item => ({ date: formatStatementDate(item.createdAt), type: 'Trade', icon: 'T', iconClass: 'vx_trade', amount: Number(item.amount) || 0, status: item.status || 'Recorded' }));

  return { deposits, withdrawals, others: trades };
};

const getPerformanceData = (userId) => getTrades()
  .filter(item => item.userId === userId && Number.isFinite(Number(item.pnl)) && Number(item.pnl) !== 0)
  .map(item => ({ date: formatStatementDate(item.createdAt), type: Number(item.pnl) > 0 ? 'win' : 'loss', amount: Number(item.pnl), side: item.side || 'trade' }));
...
router.post('/trade', async (req, res) => {
  ...
  setImmediate(() => sendTradeEmail(user.email, user.fullName, tradeAmount, signal)
    .catch((emailErr) => console.error('Trade email error:', emailErr)));
...
router.post('/deposit', (req, res) => {
  ...
  setImmediate(() => sendDepositEmail(user.email, user.fullName, depositRequest.amount, cryptocurrency)
    .catch((emailErr) => console.error('Deposit email error:', emailErr)));
...
router.post('/withdraw', (req, res) => {
  ...
  setImmediate(() => sendWithdrawalEmail(user.email, user.fullName, withdrawAmount, bitcoinAddress)
    .catch((emailErr) => console.error('Withdrawal email error:', emailErr)));
...
router.get('/account-statement', (req, res) => {
  const user = req.currentUser;
  if (!user) { req.session.destroy(); return res.redirect('/login'); }
  return res.render('user/account-statement', { user, page: 'account-statement', accountStatementData: getAccountStatementData(user.id) });
});
...
router.get('/performance-history', (req, res) => {
  const user = req.currentUser;
  if (!user) { req.session.destroy(); return res.redirect('/login'); }
  return res.render('user/performance-history', { user, page: 'performance-history', performanceData: getPerformanceData(user.id) });
});
...
router.get('/referral-program', async (req, res) => {
  const user = req.currentUser;
  if (!user) { req.session.destroy(); return res.redirect('/login'); }
  const referrals = await getUsersCollection();
  const referredUsers = await referrals.find({ referredBy: user.id }, { projection: { email: 1, createdAt: 1, id: 1 } }).toArray();
  const referralRecords = await getCollection('referrals');
  const rewards = await referralRecords.find({ referrerId: user.id }).sort({ createdAt: -1 }).toArray();
  const appUrl = (process.env.APP_URL || process.env.RENDER_EXTERNAL_URL || 'http://localhost:3000').replace(/\/$/, '');
  return res.render('user/referral-program', {
    user,
    page: 'referral-program',
    referralLink: `${appUrl}/ref/${encodeURIComponent(user.id)}`,
    referralActivity: referredUsers.map(referred => {
      const reward = rewards.find(item => item.referredId === referred.id);
      return { date: formatStatementDate(reward?.createdAt || referred.createdAt), email: referred.email, action: reward?.type === 'signup' ? 'Signup' : 'Referral activity', amount: Number(reward?.amount) || 0 };
    })
  });
});
...
// Update profile
router.post('/profile/update', (req, res) => profileUpload.single('profilePhoto')(req, res, async (uploadError) => {
  const user = req.currentUser;
  if (!user) { req.session.destroy(); return res.redirect('/login'); }
  if (uploadError) {
    console.error('Profile photo upload error:', uploadError.message);
    return res.status(400).send('Unable to save profile changes. Please choose an image smaller than 2 MB.');
  }

  try {
    const { fullName, phone, country } = req.body;
    const trimmedName = String(fullName || '').trim();
    if (!trimmedName) return res.status(400).send('Full name is required.');
    ...
    // Keep the existing photo unless the user selected a replacement.
    if (req.file) {
      updateFields.profilePhoto = `data:${req.file.mimetype};base64,${req.file.buffer.toString('base64')}`;
    }

    const users = await getUsersCollection();
    await users.updateOne({ id: user.id }, { $set: updateFields });
    return res.redirect('/user/profile-settings?saved=1');
  } catch (error) {
    console.error('Profile update error:', error);
    return res.status(500).send('Unable to save profile changes. Please try again.');
  }
}));
